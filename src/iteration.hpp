 ...
 * a shared pointer. In short, \p that instance should outlive the descent, so make
 * sure you do not disown it during the descent.
 *
 * Range-based loops should fit the vast majority of the use cases:
 * \code
 * const types::Environment env = ...;
 * for (const std::shared_ptr<types::IClass>& instance : *Descend(env)) {
 ...
 * \code
 * const types::Environment env = ...;
 * for (const std::shared_ptr<types::IClass>& instance : *DescendOnce(env)) {
 *   do_something(instance);
 * }
 * \endcode
 *
 * \param that instance to be iterated over non-recursively
 * \return Iterable over instances directly referenced from \p instance
 */
///@{
