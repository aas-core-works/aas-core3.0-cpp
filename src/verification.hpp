...
/**
 * Verify that \p that instance conforms to the meta-model constraints.
 *
 ...
 * for (const Error& error : *Verify(env)) {
 *   report_somehow(error);
 * }
 * \endcode
 *
 * \param that instance to be verified
 * \param recursively if set, verify also the instances referenced by \p that
 * instance
...
/// \brief Check that \p text is a `xs:dateTime`.
///
